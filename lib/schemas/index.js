...
function localBusiness(shop, { siteUrl, canonical, descriptionShort }) {
  ...
  // sameAs: the business's other verified identity profiles (social pages).
  // `url` above already carries the website, so it's excluded here to avoid
  // repeating the same identity. google_maps is a listing URL, not an identity
  // profile, so it is deliberately NOT included.
  const sameAs = [
    shop.website, shop.instagram_url, shop.facebook_url,
    shop.twitter_url, shop.youtube_url, shop.tiktok_url,
  ].filter(u => u && String(u).trim() !== '');
  const profiles = sameAs.filter(u => u !== shop.website);
  if (profiles.length) out.sameAs = profiles;
  if (shop.image || shop.photo_url) out.image = [shop.image || shop.photo_url];
  return out;
}
