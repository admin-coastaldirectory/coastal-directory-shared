...
function cityFAQ(city, stateCode, shops, config) {
  ...
  faqs.push({
    q: `How many ${nounPl} are in ${city}?`,
    a: `There are ${shops.length} verified ${nounPl} in ${city}, ${sn}. Every listing on this page includes the shop's address, phone number, hours, and services.`,
  });

  faqs.push({
    q: `What do ${nounPl} in ${city} offer?`,
    a: `${city} ${nounPl} typically offer ${services}. Each shop listing above shows the specific services that location provides.`,
  });
  ...
  if (rated.length) {
    ...
    faqs.push({
      ...
      a: `${top.name} is among the highest-rated ${nounPl} in ${city}, with ${stars} stars from verified Google reviews. Compare ratings across every ${city} listing above to find the best fit for what you collect.`,
    });
  }

  faqs.push({
    q: `How do I find a ${noun} near me in ${city}?`,
    a: `This page lists every verified ${noun} in ${city}, ${sn}, each with its address and opening hours, so you can find one close to you.`,
  });
...
function stateFAQ(stateCode, shops, config) {
  ...
  faqs.push({
    q: `How many ${nounPl} are in ${name}?`,
    a: `There are ${shops.length} verified ${nounPl} in ${name}, organized by city on this page so you can browse the area closest to you.`,
  });
  ...
  if (cities.length) {
    const cityList = cities.length > 5
      ...
      : cities.join(', ');
    faqs.push({
      q: `Which ${name} cities have ${nounPl}?`,
      a: `${name} ${nounPl} are spread across ${cities.length} cities and towns, ${cityList}. Use the city links above to jump to a specific area.`,
    });
  }

  faqs.push({
    q: `What do ${name} ${nounPl} offer?`,
    a: `${name} ${nounPl} offer ${services}. Each shop listing shows its specific services, opening hours, and contact details.`,
  });

  faqs.push({
    q: `How do I find a ${noun} near me in ${name}?`,
    a: `Browse the city list above to jump to your area, or scroll the full directory of ${name} ${nounPl} further down the page — each listing includes an address and hours.`,
  });
