...
function cityFAQ(city, stateCode, shops, config) {
  ...
  faqs.push(faqEntry(
    ...
    `There are ${shops.length} verified ${nounPl} in ${city}, ${sn}. Every listing on this page includes the shop's address, phone number, and services.`
  ));
  ...
  faqs.push(faqEntry(
    ...
    `This page lists every verified ${noun} in ${city}, ${sn}, each with its address and contact details, so you can find one close to you.`
  ));

  // Sell/appraisal FAQs are niche-driven via config.niche.sell; defaults reproduce the original coin wording.
  const sell = niche.sell || {};
  const sellNoun = sell.noun || 'coins';
  const sellFaqItems = sell.faqItems || 'individual coins, paper currency, and entire collections';
  const appraisalNoun = sell.appraisalNoun || 'a coin appraisal';
  const appraisalPlural = sell.appraisalPlural || 'coin appraisals';
  const appraisalBring = sell.appraisalBring || 'Bring the coins uncleaned, and ask whether the shop charges an appraisal fee.';

  faqs.push(faqEntry(
    `Can I sell ${sellNoun} in ${city}?`,
    `Yes — many ${city} ${nounPl} buy ${sellFaqItems}, not just sell them. Look for listings tagged Buy/sell or Appraisals above, and call ahead to confirm what a shop is currently buying.`
  ));

  faqs.push(faqEntry(
    `Where can I get ${appraisalNoun} in ${city}?`,
    `Several ${city} ${nounPl} offer in-person ${appraisalPlural}. An appraisal tells you what your ${sellNoun} are realistically worth before you sell. ${appraisalBring}`
  ));
...
// ============================================================
//
// Visible FAQ section + FAQPage schema for the homepage — generic,
// site-wide questions (not city/shop-specific). City/shop pages
// already had FAQPage; the homepage was the gap the audit flagged,
// and it's the strongest AI-citation predictor available site-wide.

function homeFAQ(config) {
  const niche = config.niche || {};
  const brand = config.brand || {};
  const noun = niche.shopType || 'shop';
  const nounPl = niche.shopTypePluralCapitalized
    ? niche.shopTypePluralCapitalized.toLowerCase()
    : noun + 's';
  const services = niche.metaServices || 'services, hours, and reviews';
  ...
  faqs.push(faqEntry(
    ...
    `Yes. ${brand.name} is a free directory of verified ${nounPl} — there's no cost to search, browse listings, or contact a shop.`
  ));

  faqs.push(faqEntry(
    `How does ${brand.name} verify its ${nounPl}?`,
    `Every listing on ${brand.name} is checked for a real address and working contact details before it's published, and shop owners can claim and update their own listing.`
  ));

  faqs.push(faqEntry(
    `What information can I find for each ${noun}?`,
    `Each listing includes the shop's address, phone number, website, hours when available, and the specific services it offers, such as ${services}.`
  ));

  faqs.push(faqEntry(
    `How do I list my ${noun} on ${brand.name}?`,
    `Use the "Add Your Shop" link to submit your ${noun} for free, or claim an existing listing if your shop is already in the directory.`
  ));
