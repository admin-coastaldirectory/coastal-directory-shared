...
function cityPage(city, stateCode, shops, config, siblingCities) {
  ...
  const sellLine = buyers
    ? `${buyers} of the ${shops.length} ${shopTypePlural} listed here buy coins, paper currency, and entire collections`
    : `Many ${shopTypePlural} in ${escHtml(city)} buy coins, paper currency, and entire collections`;
  const apprLine = appraisers ? `, and ${appraisers} offer in-person appraisals` : '';
  const sellAppraiseHtml =
    `<h2>Selling or appraising coins in ${escHtml(city)}?</h2>\n`
    + `<p>A coin shop isn't only for buying. ${sellLine}${apprLine}. `
    + `If you're thinking of selling, it's worth getting an appraisal first so you know what your coins are really worth — and never clean a coin beforehand, since cleaning lowers its value. `
