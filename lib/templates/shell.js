...
const { escHtml, escAttr, marketplaceLinks } = require('../util');
const { isAggregatorUrl } = require('../schemas');

// ------------------------------------------------------------
// forceOrigin — pins a URL's protocol+host to config.brand.fullUrl
// ------------------------------------------------------------
//
// Defends against a wrong/drifted domain (e.g. a hardcoded canonical
// left over from a stale hand-written page, like the CoinsNearMe
// .com/.co bug) or a bare path being fed into pageShell(). Always
// returns an absolute URL on the configured brand origin, preserving
// path/query/hash from the input. Exported so callers (and the
// verify script) can reuse it directly.

function forceOrigin(url, brand) {
  const fullUrl = String((brand && brand.fullUrl) || '').replace(/\/$/, '');
  if (!fullUrl) return url;
  if (!url) return fullUrl + '/';
  let rest;
  try {
    // Absolute URL on any host — keep path+query+hash, drop the origin.
    const u = new URL(url, fullUrl + '/');
    rest = u.pathname + u.search + u.hash;
  } catch (e) {
    // Not parseable as a URL — treat it as a bare path.
...
//   robots:       optional robots directive (defaults to 'index, follow' so
//                 every existing caller is unaffected)
//   showCdn:      pass false to suppress the Coastal Directory Network
//                 cross-domain band. Google's spam policy names "widely
//                 distributed links in the footers or templates of various
//                 sites" verbatim, so the band must NOT ship on the ~18,000
//                 generated shop/city/state pages. Omitting the flag keeps
//                 the band (homepage, /about, /advertise).

function pageShell({ config, title, description, canonical, schema, body, ogImage, robots, showCdn }) {
...
gtag('config','${escAttr(ga)}');
  ...
  // --- Meta pixel -------------------------------------------------
  // Set config.analytics.metaPixel to the dataset ID. Omit it and
  // nothing is emitted, so every existing config is unaffected.
  //
  // Advanced Matching: we pass `em` when a buyer/subscriber email has
  // been stashed by a form on an earlier page. The browser pixel
  // normalizes and SHA-256 hashes it client-side, so no raw email is
  // ever sent and no hashing code is needed here.
  //
  // fbc: fbclid only lands on the FIRST page of a visit. Without this
  // it is lost on every subsequent hop, which is why Lead fbc coverage
  // sat at 20% vs 46% on PageView. We persist it into the _fbc cookie
  // in Meta's documented format so it survives the whole funnel.
  const metaPixel = config.analytics && config.analytics.metaPixel;
  const cat = (config.analytics && config.analytics.metaCategory) || '';
  const metaPixelSnippet = metaPixel ? `<script>
...
(function(){
  try{
    ...
    if(c&&!/(^|;\\s*)_fbc=/.test(document.cookie)){
      document.cookie='_fbc=fb.1.'+Date.now()+'.'+c+';path=/;max-age=7776000;SameSite=Lax';
    }
  }catch(e){}
  var am={};
  try{var em=sessionStorage.getItem('cd_em');if(em){am.em=em;}}catch(e){}
  fbq('init','${escAttr(metaPixel)}',am);
  fbq('track','PageView'${cat ? `,{content_category:'${escAttr(cat)}'}` : ''});
})();
</script>
<noscript><img height="1" width="1" style="display:none" alt="" src="https://www.facebook.com/tr?id=${escAttr(metaPixel)}&ev=PageView&noscript=1"/></noscript>` : '';

  const ogImg = ogImage || `${brand.fullUrl}/og-default.png`;
  // Canonical + og:url are always forced onto the configured brand
  // origin, regardless of what the caller passed in — this is what
  // makes the CoinsNearMe .com/.co drift structurally impossible on
  // any page that goes through pageShell().
  const canonicalUrl = forceOrigin(canonical, brand);
...
<meta name="robots" content="${escAttr(robots || 'index, follow')}">
...
.shop-online{display:flex;gap:6px;flex-wrap:wrap;margin:0 0 10px}
.chip{font-size:11px;font-weight:600;padding:4px 10px;border-radius:12px;display:inline-flex;align-items:center;gap:4px;line-height:1.4}
.chip.ebay{color:#00478f;background:#e8f1fd}
.chip.whatnot{color:#6a1fb0;background:#f3e8fd}
.shop-online-links{margin:24px 0 0}
.shop-online-links h2{margin-bottom:6px}
.shop-online-links .links{display:flex;gap:10px;flex-wrap:wrap;margin-top:12px}
...
.social-row a{color:#8a8a8a;display:inline-flex;transition:color .15s}
.social-row a:hover{color:${accent}}
.social-row svg{width:20px;height:20px;fill:currentColor;display:block}
`;
}

// ------------------------------------------------------------
// socialRow — Facebook + Instagram icon links (from config.social)
// ------------------------------------------------------------
//
// Renders nothing if config.social is absent. Icons inherit footer
// color and use the shared .social-row CSS above.

function socialRow(config) {
  const s = (config && config.social) || {};
  const links = [];
  if (s.facebook) {
    links.push(`<a href="${escAttr(s.facebook)}" target="_blank" rel="noopener" aria-label="Facebook"><svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M22 12.06C22 6.5 17.52 2 12 2S2 6.5 2 12.06c0 5.02 3.66 9.19 8.44 9.94v-7.03H7.9v-2.91h2.54V9.85c0-2.52 1.49-3.91 3.78-3.91 1.09 0 2.24.2 2.24.2v2.47h-1.26c-1.24 0-1.63.78-1.63 1.57v1.88h2.78l-.44 2.91h-2.34V22c4.78-.75 8.43-4.92 8.43-9.94z"/></svg></a>`);
  }
  if (s.instagram) {
    links.push(`<a href="${escAttr(s.instagram)}" target="_blank" rel="noopener" aria-label="Instagram"><svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M12 2.16c3.2 0 3.58.01 4.85.07 1.17.05 1.8.25 2.23.41.56.22.96.48 1.38.9.42.42.68.82.9 1.38.16.42.36 1.06.41 2.23.06 1.27.07 1.65.07 4.85s-.01 3.58-.07 4.85c-.05 1.17-.25 1.8-.41 2.23-.22.56-.48.96-.9 1.38-.42.42-.82.68-1.38.9-.42.16-1.06.36-2.23.41-1.27.06-1.65.07-4.85.07s-3.58-.01-4.85-.07c-1.17-.05-1.8-.25-2.23-.41-.56-.22-.96-.48-1.38-.9-.42-.42-.68-.82-.9-1.38-.16-.42-.36-1.06-.41-2.23C2.17 15.58 2.16 15.2 2.16 12s.01-3.58.07-4.85c.05-1.17.25-1.8.41-2.23.22-.56.48-.96.9-1.38.42-.42.82-.68 1.38-.9.42-.16 1.06-.36 2.23-.41C8.42 2.17 8.8 2.16 12 2.16M12 0C8.74 0 8.33.01 7.05.07 5.78.13 4.9.33 4.14.63c-.79.31-1.46.72-2.12 1.38C1.36 2.67.95 3.34.63 4.14.33 4.9.13 5.78.07 7.05.01 8.33 0 8.74 0 12s.01 3.67.07 4.95c.06 1.27.26 2.15.56 2.91.31.79.72 1.46 1.38 2.12.66.66 1.33 1.07 2.12 1.38.76.3 1.64.5 2.91.56C8.33 23.99 8.74 24 12 24s3.67-.01 4.95-.07c1.27-.06 2.15-.26 2.91-.56.79-.31 1.46-.72 2.12-1.38.66-.66 1.07-1.33 1.38-2.12.3-.76.5-1.64.56-2.91.06-1.28.07-1.69.07-4.95s-.01-3.67-.07-4.95c-.06-1.27-.26-2.15-.56-2.91-.31-.79-.72-1.46-1.38-2.12-.66-.66-1.33-1.07-2.12-1.38-.76-.3-1.64-.5-2.91-.56C15.67.01 15.26 0 12 0zm0 5.84A6.16 6.16 0 1018.16 12 6.16 6.16 0 0012 5.84zM12 16a4 4 0 114-4 4 4 0 01-4 4zm6.41-10.85a1.44 1.44 0 101.44 1.44 1.44 1.44 0 00-1.44-1.44z"/></svg></a>`);
  }
  const xUrl = s.x || s.twitter;
  if (xUrl) {
    links.push(`<a href="${escAttr(xUrl)}" target="_blank" rel="noopener" aria-label="X"><svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M18.901 1.153h3.68l-8.04 9.19L24 22.846h-7.406l-5.8-7.584-6.638 7.584H.474l8.6-9.83L0 1.154h7.594l5.243 6.932ZM17.61 20.644h2.039L6.486 3.24H4.298Z"/></svg></a>`);
  }
  if (s.reddit) {
    links.push(`<a href="${escAttr(s.reddit)}" target="_blank" rel="noopener" aria-label="Reddit"><svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M12 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0zm5.01 4.744c.688 0 1.25.561 1.25 1.249a1.25 1.25 0 0 1-2.498.056l-2.597-.547-.8 3.747c1.824.07 3.48.632 4.674 1.488.308-.309.73-.491 1.207-.491.968 0 1.754.786 1.754 1.754 0 .716-.435 1.333-1.01 1.614a3.111 3.111 0 0 1 .042.52c0 2.694-3.13 4.87-7.004 4.87-3.874 0-7.004-2.176-7.004-4.87 0-.183.015-.366.043-.534A1.748 1.748 0 0 1 4.028 12c0-.968.786-1.754 1.754-1.754.463 0 .898.196 1.207.49 1.207-.883 2.878-1.43 4.744-1.487l.885-4.182a.342.342 0 0 1 .14-.197.35.35 0 0 1 .238-.042l2.906.617a1.214 1.214 0 0 1 1.108-.701zM9.25 12C8.561 12 8 12.562 8 13.25c0 .687.561 1.248 1.25 1.248.687 0 1.248-.561 1.248-1.249 0-.688-.561-1.249-1.249-1.249zm5.5 0c-.687 0-1.248.561-1.248 1.25 0 .687.561 1.248 1.249 1.248.688 0 1.249-.561 1.249-1.249 0-.687-.562-1.249-1.25-1.249zm-5.466 3.99a.327.327 0 0 0-.231.094.33.33 0 0 0 0 .463c.842.842 2.484.913 2.961.913.477 0 2.105-.056 2.961-.913a.361.361 0 0 0 .029-.463.33.33 0 0 0-.464 0c-.547.533-1.684.73-2.512.73-.828 0-1.979-.196-2.512-.73a.326.326 0 0 0-.232-.095z"/></svg></a>`);
  }
...
function shopCard(shop, config, stSlug) {
  ...
  // eBay / Whatnot chips. marketplaceLinks() returns null for any shop that
  // isn't featured, so a non-featured shop with the links stored shows nothing.
  // These are spans, not anchors: the whole card is already an <a>, and a
  // nested <a> is invalid HTML. The real outbound links live on the shop page.
  const marketplace = marketplaceLinks(shop);
  const marketplaceChips = marketplace
    ? `<div class="shop-online">${marketplace.ebay ? '<span class="chip ebay">eBay store</span>' : ''}${marketplace.whatnot ? '<span class="chip whatnot">Whatnot</span>' : ''}</div>`
    : '';
...
function eventCard(ev) {
  ...
  const day = d.getDate();
  // Never link a card straight to a scrape-source aggregator (e.g. a
  // third-party show calendar) — that trains readers (and crawlers) to
  // treat the aggregator as the authority. Fall back to a maps link on
  // the venue/address so the card is still useful, rather than a dead '#'.
  const realUrl = ev.website && !isAggregatorUrl(ev.website) ? ev.website : null;
  const addr = [ev.venue, ev.address, ev.city, ev.state].filter(Boolean).join(', ');
  const mapsUrl = addr ? `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(addr)}` : null;
  const href = realUrl || mapsUrl || '#';
  return `<a href="${escAttr(href)}" class="event-card" target="_blank" rel="noopener">
...
module.exports = { pageShell, header, footer, ctaBanner, cdnBand, shopCard, eventCard, socialRow, forceOrigin };
