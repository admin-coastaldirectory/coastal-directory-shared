...
function shopPage(shop, config, allStateShops) {
  ...
  // Fallback to 'Other' must match generate.js (line ~113) so canonical
  // URL agrees with file path on disk. Shops missing a city land in
  // /shops/<state>/other/<shop>/ instead of /shops/<state>//<shop>/.
  const citySlug = slugify(shop.city || 'Other');
  ...
  const claimCtaHtml = '<aside class="claim-cta">\n'
    + ' <strong>Is this your shop? Claim it free to update hours, photos, and services.</strong>\n'
    + ' <a href="/claim.html?shop=' + escAttr(shopSlug) + '" class="btn outline">Claim free →</a>\n'
    + '</aside>';
