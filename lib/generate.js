...
const { statePage, cityPage, eventStatePage, shopsIndexPage } = require('./templates/listing-page');
const { featuredPage, featuredBadgeSvg } = require('./templates/featured-page');
...
async function build(config) {
  ...
  // Static /shops/ hub — server-rendered directory (state grid + top cities +
  // ItemList/BreadcrumbList JSON-LD). Written after the state loop so the wiped
  // shops/ dir already holds every state/city/shop page it links to. Its URL
  // (siteUrl + '/shops/') is already in coreUrls, so it's in sitemap-core.
  const hubRes = shopsIndexPage(shops, byState, config);
  writeFile(path.join(outDir, 'shops', 'index.html'), hubRes.html);
  console.log('  shops hub:          /shops/ (static)');

  // /featured/ badge program page + the brand badge SVG every embed references.
  // The badge is only *offered* to Tier A shops (the CTA on their pages links
  // here); this page also builds a snippet from any pasted shop URL.
  const featRes = featuredPage(config);
  writeFile(path.join(outDir, 'featured', 'index.html'), featRes.html);
  writeFile(path.join(outDir, 'featured-badge.svg'), featuredBadgeSvg(config));
  console.log('  featured page:      /featured/ + /featured-badge.svg');
